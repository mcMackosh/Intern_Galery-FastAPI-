...
class ImageService:
    ...
    async def upload_images(
        ...
        for file in files:
            ...
            if ext not in ALLOWED_EXTENSIONS:
                raise HTTPException(status_code=400, detail=f"File '{file.filename}' has unsupported extension")
            if file.content_type not in ALLOWED_MIME_TYPES:
                raise HTTPException(status_code=400, detail=f"File '{file.filename}' has unsupported MIME type")
    ...
    async def delete_images(self, db: AsyncSession, ids: list[str], gallery_id: str):
        ...
        if len(images) != len(ids):
            raise HTTPException(status_code=403, detail="Some images do not belong to this gallery")
    ...
    async def copy_images(
        ...
        for image in images:
            ...
            new_image = await image_repository.create(db, target_gallery_id, new_relative, image.original_filename)
